...
import { clear, closeOrder, closePayment, openPayment } from "../store/reducer/carrinho";
...
const Order = () => {
    ...
    const [purchase, { data, isLoading, error, isSuccess }] = usePurchaseMutation();
    ...
    const getTotalPrice = () => {
        return items.reduce((acumulador: number, item: CartItem) => {
            return acumulador + (item.preco * item.quantidade);
        }, 0);
    };

    const getProductsForAPI = () => {
    ...
    console.log('API Status:', { isLoading, error, isSuccess, data });
                ...
                <>
                    {isPayment ? (
                        <form id="paymentForm" onSubmit={formikPagamento.handleSubmit}>
                            <S.OrderTitle>Pagamento - valor a pagar R$ {getTotalPrice().toFixed(2)}</S.OrderTitle>
